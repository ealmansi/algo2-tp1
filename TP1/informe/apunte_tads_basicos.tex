...
\tadGeneros{bool}
\tadExporta{bool, generadores, $\neg$, $\lor$, $\land$, $\implies$, \IF \argumento THEN \argumento ELSE \argumento FI}
...
\tadIgualdadObservacional{n}{m}{nat}{$(n=0? \igobs m=0?)\ \land$\\ $(\neg(n=0?) \implies (\text{pred}(n) \igobs \text{pred}(m)))$}
...
\tadOperacion{\argumento $-$ \argumento}{nat/n,nat/m}{nat}{$m \opsub{\leq}{nat} n$}
...
\tadAxioma{$n < m$}{$\lnot(m=0?)$ $\land$ $(n=0? \lor\ \text{pred}(n)<\text{pred}(m))$}
...
\tadIgualdadObservacional{s}{s'}{secu($\alpha$)}{vacía?($s$) $\igobs$ vacía?($s'$) $\land$ \\($\neg$ vacía?($s$) $\implies$ (prim($s$) $\igobs$ prim($s'$) $\land$ fin($s$) $\igobs$ fin($s'$))}
...
\tadAxioma{está?($e$, $s$)}{$\neg$ vacía?($s$) $\land$\ ($e =$ prim($s$) $\lor$ está?($e$, fin($s$))}
...
\tadOperacion{\argumento [ \argumento] $\leftarrow$ \argumento}{ad($\alpha$)/a,nat/n,$\alpha$}{ad($\alpha$)}{$n$ \opsub{<}{nat} tam($a$)}
...
\tadIgualdadObservacional{p}{p'}{pila($\alpha$)}{vacía?($p$) $\igobs$ vacía?($p'$)) $\land$ ($\neg$ vacía?($p$) $\implies$\\ (tope($p$) $\igobs$ tope($p'$) $\land$ desapilar($p$) $\igobs$ desapilar($p'$))}
...
\tadIgualdadObservacional{c}{c'}{cola($\alpha$)}{vacía?($c$) $\igobs$ vacía?($c'$) $\land$ \\ 
                                                 ($\neg$vacía?($c$) $\implies$ (próximo($c$) $\igobs$ próximo($c'$) $\land$ \\
...
\tadIgualdadObservacional{a}{a'}{ab($\alpha$)}{nil?($a$) $\igobs$ nil?($a'$) $\land$ ($\neg$ nil?($a$) $\implies$ (raiz($a$) $\igobs$ raiz($a'$) $\land$ izq($a$) $\igobs$ izq($a'$) $\land$ der($a$) $\igobs$ der($a'$))}
...
\tadIgualdadObservacional{d}{d'}{dicc($\kappa,\sigma$)}{$(\forall c:\kappa)$(def?($c, d$) $\igobs$ def?($c, d'$) $\land$\\ (def?($c, d$) $\implies$ obtener($c, d$) $\igobs$ obtener($c, d'$)))}
...
\tadIgualdadObservacional{c}{c'}{colaPrior($\alpha$)}{vacía?($c$) $\igobs$ vacía?($c'$) $\land$ \\ 
                                                 ($\neg$vacía?($c$) $\implies$ (próximo($c$) $\igobs$ próximo($c'$) $\land$ \\
                                        \phantom{($\neg$vacía?($c$) $\implies$ (}desencolar($c$) $\igobs$ desencolar($c'$))}
...
\tadAxioma{próximo(encolar($e$, $c$))}{\IF vacía?($c$) $\lor$\ proximo($c$) $< e$ THEN $e$ ELSE próximo($c$) FI}

\tadAxioma{desencolar(encolar($e$, $c$))}{\IF vacía?($c$) $\lor$\ proximo($c$) $< e$ THEN $c$ ELSE encolar($e$, desencolar($c$)) FI}
